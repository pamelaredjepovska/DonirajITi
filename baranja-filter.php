...
/* da gi sortira i ogranici postovite na 4 */
	...
	{
		header("Location: prikazibaranja.php");
		//$sql = "SELECT * FROM objavi WHERE Tip='Baranje' ORDER BY Datum desc"; 
	}
	else if ($grad == 'G' && $kategorija != 'K'){
		$sql = "SELECT * FROM objavi WHERE Tip='Baranje' AND KategorijaID = $kategorija ORDER BY Datum desc"; 
	}
	else if ($grad != 'G' && $kategorija == 'K'){
		$sql = "SELECT * FROM objavi WHERE Tip='Baranje' AND Lokacija = '$grad' ORDER BY Datum desc"; 
	}
	else 
	{
		$sql = "SELECT * FROM objavi WHERE Tip='Baranje' AND Lokacija = '$grad' AND KategorijaID = $kategorija ORDER BY Datum desc"; 
	}
...
<div class="blog-latest"> 
    ...
    <div class="d-flex flex-row-reverse align-items-center" style="padding-right:0px">
        <div class="p-2"><a class="btn btn-primary btn-xl" href="novobaranje.php">Објави барање</a></div>
    ...
    <div class="main-carousel p-2" id="latestCarousel"> 
        ...
        <?php 
           ...
           // $sql = "SELECT COUNT(*) FROM objavi WHERE Tip='Donacija' ORDER BY Datum desc";
            if($grad == 'G' && $kategorija == 'K')
            {
            $sql = "SELECT COUNT(*) FROM objavi WHERE Tip='Baranje' ORDER BY Datum desc"; 
            }
            else if ($grad == 'G' && $kategorija != 'K'){
                $sql = "SELECT COUNT(*) FROM objavi WHERE Tip='Baranje' AND KategorijaID = $kategorija ORDER BY Datum desc"; 
            }
            else if ($grad != 'G' && $kategorija == 'K'){
                $sql = "SELECT COUNT(*) FROM objavi WHERE Tip='Baranje' AND Lokacija = '$grad' ORDER BY Datum desc"; 
            }
            else 
            {
                $sql = "SELECT COUNT(*) FROM objavi WHERE Tip='Baranje' AND Lokacija = '$grad' AND KategorijaID = $kategorija ORDER BY Datum desc"; 
            }
           ...
           // $sql = "SELECT * FROM objavi WHERE Tip='Donacija' ORDER BY Datum desc LIMIT $offset, $rowsperpage";
            if($grad == 'G' && $kategorija == 'K')
            {
            $sql = "SELECT * FROM objavi WHERE Tip='Baranje' ORDER BY Datum desc LIMIT $offset, $rowsperpage"; 
            }
            else if ($grad == 'G' && $kategorija != 'K'){
                $sql = "SELECT * FROM objavi WHERE Tip='Baranje' AND KategorijaID = $kategorija ORDER BY Datum desc LIMIT $offset, $rowsperpage"; 
            }
            else if ($grad != 'G' && $kategorija == 'K'){
                $sql = "SELECT * FROM objavi WHERE Tip='Baranje' AND Lokacija = '$grad' ORDER BY Datum desc LIMIT $offset, $rowsperpage"; 
            }
            else 
            {
                $sql = "SELECT * FROM objavi WHERE Tip='Baranje' AND Lokacija = '$grad' AND KategorijaID = $kategorija ORDER BY Datum desc LIMIT $offset, $rowsperpage"; 
            }
            $result = mysqli_query($conn, $sql);// or trigger_error("SQL", E_USER_ERROR);
            ...
            <div class="card mx-4" style="width:30rem; height:400px; object-fit:cover;"> 
                ...
                <p class="card-subtitle mt-2 text-muted"> 
                    ...
                    <p style="font-size:18px; position: absolute; bottom: 0; right: 15px;">
                        ... <a href="view.php?postid=<?=$id?>">Read More</a></p>
                </p> 
                </div> 
            </div> 
            <?php echo "<br>";?>
