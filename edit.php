...
<?php 
  ...
  //dokolku nema parametar za objava id vo linkot da prenasoci na pocetnata strana
...
$idpost = $_GET['postid'];
$iduser = $_SESSION['id'];
$q = "SELECT Naslov,Sodrzina FROM Objavi WHERE Objava_ID = $idpost AND KorisnikID = $iduser";
$rez = mysqli_query($conn,$q);
if(mysqli_num_rows($rez)==0)
{
    header("Location:error.php"); //prenasocuvanje dokolku baranata objava ne postoi
}
$red = mysqli_fetch_row($rez);
$starnaslov = $red[0];
$starasodrzina = $red[1];
  ...
  {
   // require('database.php');
     $objava = mysqli_real_escape_string($conn,$_POST['id']); //zemanje na vrednosta za parametarot za id na objavata koja treba da se prikaze
    ...
    if(!empty($_FILES["file"]["name"]))
    {
      ...
      $targetFilePath = $targetDir . $fileName; //patekata na foldero kade ke se smesti slikata
      ...
      if(in_array($fileType,$allowTypes)) //ako slikata e dozvolen format
      {
        ...
        {       
          $query = "UPDATE objavi SET Naslov = '$naslov', Sodrzina = '$sodrzina', Lokacija = '$lokacija', Picture = '$fileName', KategorijaID = $kategorijaID WHERE Objava_ID = $objava";
...
<html lang="en">
  ...
  <body>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      <div class="container">
        <a class="navbar-brand mr-auto" href="homepage.html"><img src="images/logo2.png" alt="Logo"></a>
    ...
    <div class="container post-holder mx-auto">
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" enctype="multipart/form-data">
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="naslov" id="Naslov" placeholder="Внесете наслов" value="<?php echo htmlspecialchars($starnaslov)?>" required>
          </div>
          <div class="form-group">
            <label for="widgEditor">Опис за донацијата:</label>
            <textarea class="form-control widgEditor" name="sodrzina" id="widgEditor" placeholder="Внесете опис" rows="10" required> <?php echo htmlspecialchars($starasodrzina)?></textarea>
          </div>
          <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input type="hidden" name="id" value="<?= $_GET['postid']?>" style="display:none;">
