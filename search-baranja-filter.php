<?php 
	...
	{
		$param = mysql_real_escape_string($_POST['srchbaranja']);
	}
	else
	{
		$param = mysql_real_escape_string($_GET['param1']);
	}
	
	if($param == "") //Ako nema vneseno nikakov parametar za prebaruvanje
	{ 
		header("Location: prikazibaranja.php");
	}
	else //Pronajdi gi zapisite koi go sodrzat parametarot vo naslovot ili sodrzinata
	{
		$sql = "SELECT * FROM objavi WHERE Tip='Baranje' AND (Naslov LIKE '%$param%' OR Sodrzina LIKE '%$param%') ORDER BY Datum desc";
...
<div class="blog-latest">
	...
	<form action="" method="post">
        <input type="hidden" name="param1" value="<?=$param?>">
    </form>
    ...
    <div class="main-carousel p-2" id="latestCarousel"> 
        ...
        <?php 
           ...
           // $sql = "SELECT COUNT(*) FROM objavi WHERE Tip='Donacija' ORDER BY Datum desc";
			...
			if($param != "")
			{
				$sql = "SELECT COUNT(*) FROM objavi WHERE Tip='Baranje' AND (Naslov LIKE '%$param%' OR Sodrzina LIKE '%$param%') ORDER BY Datum desc";
            }
           ...
           // $sql = "SELECT * FROM objavi WHERE Tip='Donacija' ORDER BY Datum desc LIMIT $offset, $rowsperpage";
			if($param != "")
			{
				$sql = "SELECT * FROM objavi WHERE Tip='Baranje' AND (Naslov LIKE '%$param%' OR Sodrzina LIKE '%$param%') ORDER BY Datum desc LIMIT $offset, $rowsperpage"; 
            }
        ...
        <?php 
                ...
                if ($currentpage > 1) {
                   ...
                   echo "<a href='{$_SERVER['PHP_SELF']}?currentpage=1&param1=$param'><<</a> ";
                   // get previous page num
                   $prevpage = $currentpage - 1;
                   // show < link to go back to 1 page
                   echo " <a href='{$_SERVER['PHP_SELF']}?currentpage=$prevpage&param1=$param'><</a> ";
                } // end if 
                ...
                for ($x = ($currentpage - $range); $x < (($currentpage + $range) + 1); $x++) {
                   ...
                   if (($x > 0) && ($x <= $totalpages)) {
                      ...
                      // if not current page...
                         ...
                         // make it a link
                         echo " <a href='{$_SERVER['PHP_SELF']}?currentpage=$x&param1=$param'>$x</a> ";
                      } // end else
                   } // end if 
                } // end for
                ...
                if ($currentpage != $totalpages) {
                   ...
                   echo " <a href='{$_SERVER['PHP_SELF']}?currentpage=$nextpage&param1=$param'>></a> ";
                   // echo forward link for lastpage
                   echo " <a href='{$_SERVER['PHP_SELF']}?currentpage=$totalpages&param1=$param'>>></a> ";
                } // end if
